...
def find_bc_index(bcs: list | tuple, key: str) -> Optional[int]:
    ...
    for iBC, bc in enumerate(bcs):
        ...
        if key[:3] == 'BC_' and key[3:] == bc.name or key[:3] == 'bc_' and key[3:] == bc.name:
            return iBC
    return None
...
def get_nonconnected_sides(sides: list, mesh: meshio.Mesh) -> tuple[tuple, tuple[npt.NDArray]]:
    ...
    nConnSide = tuple(
        s
        for s in sides
        if s.connection is None  # noqa: E271
        and (s.bcid is None or mesh_vars.bcs[s.bcid].type[0] in (0, 1, 100, -100))
    )
...
def ConnectMesh() -> None:
    ...
    csetMap = {
        key: tuple(i for i, cell in enumerate(cset) if cell is not None and cast(np.ndarray, cell).size > 0)
        ...
        for iMap in csetMap[key]:
            ...
            for iSide in iBCsides:
                ...
                match cast(np.ndarray, bcs[bcID].type)[0]:
                    ...
                    case _:  # Boundary side
                        ...
                        if len(sideIDs) > 1 and checkInternalBoundaries:
                            hopout.error(f'Found internal face with multiple BCs [{bcID}], exiting...', traceback=True)
                ...
                for sideID in sideIDs:
                    ...
                    if cast(np.ndarray, bcs[bcID].type)[0] == 1 and cast(np.ndarray, bcs[bcID].type)[3] > 0:
                    ...
                    if cast(np.ndarray, bcs[bcID].type)[0] not in (0, 1, 100, -100):
                        bar.step()
        ...
        match len(val):
            ...
            case 2:  # Internal side
                ...
                if side0.bcid is not None:
                    ...
                    if bcs[side0.bcid].type[0] not in (0, 1, 100, -100):
                        continue
    ...
    for i, s in enumerate(sides):
        ...
        sides_bc[i] = s.bcid is not None and bcs[s.bcid].type[0] != 100 and bcs[s.bcid].type[0] != -100  # noqa: E272
