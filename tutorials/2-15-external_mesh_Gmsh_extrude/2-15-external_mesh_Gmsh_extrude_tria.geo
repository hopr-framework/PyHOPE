...
Point(3) = {0,  1,  0, lc};             // Corner 3

Line(1) = {1, 2};                       // Bottom edge
Line(2) = {2, 3};                       // Right edge
Line(3) = {3, 1};                       // Left edge

Curve Loop(1) = {1, 2, 3};              // Boundary loop for the surface
Plane Surface(1) = {1};                 // Create the planar surface bounded by Curve Loop(1)

Transfinite Curve {1,2,3} = 2;          // 2 points per edge (endpoints only) -> 1 division -> 1 element along each edge
Transfinite Surface {1} = {1,2,3};      // Use a structured transfinite map on the surface
